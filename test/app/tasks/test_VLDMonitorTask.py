...
class TestVLDHWTests:
    def test_when_pin_pulled_high_then_low(self):
        bsp.bsp_init()
        queue = SessionQueue()
        vld = VLDMonitor(queue)
        vld.start()
        ...
        q = queue.get()
        assert q.type == EventType.VEHICLE_DETECTED

        q = queue.get()
        assert q.type == EventType.VEHICLE_GONE
